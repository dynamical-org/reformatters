...
class EcccHrdpsForecastTemporalTemplateConfig(TemplateConfig[EcccHrdpsDataVar]):
    ...
    def dataset_attributes(self) -> DatasetAttributes:
        return DatasetAttributes(
            ...
            description="Weather forecast data from the High Resolution Deterministic "
            "Prediction System (HRDPS) continental domain, operated by Environment "
            "and Climate Change Canada (ECCC).",
            attribution="ECCC HRDPS Forecast data processed by dynamical.org from "
    ...
    def data_vars(self) -> Sequence[EcccHrdpsDataVar]:
        ...
        Empty for now: the real variable set (temperature_2m, precipitation_surface,
        etc. - see reformatters#711 source data exploration) is still being designed.
        Left empty (rather than NotImplementedError) so this dataset can be registered
        in DYNAMICAL_DATASETS for its archive_grib_files command/cron - `data_vars` is
        read unconditionally by DynamicalDataset's model validator. `coords` and
        `dimension_coordinates` below are the real "not implemented yet" markers.
        """
        return ()
