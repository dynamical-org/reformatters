...
class ExampleRegionJob(VirtualRegionJob[ExampleDataVar, ExampleSourceFileCoord]):
    # Concurrent source file fetches while building refs (file_refs runs in a thread
    # pool). The base default is 32; raise it when refs come from many small latency
    # bound requests like .idx sidecars.
    #
    ...
    def generate_source_file_coords(
        ...
        Same role as in a materialized dataset: enumerate the source files. Reused by
        operational validation to probe the manifest, so it must list exactly the files
        the dataset expects (drop files the source genuinely lacks, e.g. accumulated
        variables at hour 0).
        """
        # return [
    ...
    def discover_available(
        self, pending: list[ExampleSourceFileCoord]
    ) -> list[tuple[ExampleSourceFileCoord, int]]:
    ...
    def file_refs(
        self, coord: ExampleSourceFileCoord, file_size: int
    ) -> list[VirtualRef]:
    ...
    #
    # def representative_var(self, coord: ExampleSourceFileCoord) -> ExampleDataVar:
