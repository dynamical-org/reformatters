...
class ExampleDataset(DynamicalDataset[ExampleDataVar, ExampleSourceFileCoord]):
    ...
    #             ),
    #         ),
    #         # Every commit rewrites each touched array's active manifest split, so the
    #         # split size caps commit cost. Sized so one split is roughly one chunk of
    #         # append-dim positions; see "Manifest splitting" in docs/virtual_datasets.md.
