...
from reformatters.common.logging import add_betterstack_log_handler
...
# Errors go to Sentry's SDK; its DSN points at Better Stack's errors application.
# Logs go to Better Stack via logtail, crons via heartbeats (see common/betterstack.py).
...
if Config.is_betterstack_logs_enabled:
    assert Config.betterstack_source_token is not None
    add_betterstack_log_handler(
        Config.betterstack_source_token, Config.betterstack_ingesting_host
    )
