...
_CYCLE_FREQUENCY = f"{whole_hours(GEFS_INIT_TIME_FREQUENCY)}h"
...
class NoaaGefsAnalysis025DegreeVirtualRegionJob(
    ...
    def generate_source_file_coords(
        ...
        """Use the shortest lead holding each variable.

        A time resolves to the cycle it falls in at lead 0 or 3; a variable with no
        hour-0 values shifts back one cycle to lead 6, since its lead 0 window is zero
        length. Variables landing on the same file share one coord, so each file is
        listed and indexed once.
        ...
        for time in pd.to_datetime(processing_region_ds["time"].values):
            cycle = time.floor(_CYCLE_FREQUENCY)
...
def _from_own_cycle(coord: NoaaGefsAnalysis025DegreeVirtualSourceFileCoord) -> bool:
    ...
    previous one."""
    return coord.init_time == coord.valid_time().floor(_CYCLE_FREQUENCY)
