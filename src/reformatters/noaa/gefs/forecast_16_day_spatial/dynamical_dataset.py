...
from .template_config import GefsForecast16DaySpatialTemplateConfig


class GefsForecast16DaySpatialDataset(
    DynamicalDataset[GEFSDataVar, GefsForecast16DaySpatialSourceFileCoord]
):
    """GEFS 16-day spatial (virtual icechunk) forecast dataset."""

    template_config: GefsForecast16DaySpatialTemplateConfig = (
        GefsForecast16DaySpatialTemplateConfig()
    )
    region_job_class: type[GefsForecast16DaySpatialRegionJob] = (
        GefsForecast16DaySpatialRegionJob
    )
    ...
    icechunk_virtual_config: IcechunkVirtualConfig = Field(
        default_factory=lambda: IcechunkVirtualConfig(
            ...
            # One init per manifest split keeps the per-commit manifest rewrite
            # small during operational updates.
    ...
    def operational_kubernetes_resources(self, image_tag: str) -> Sequence[CronJob]:
        ...
        # it publishes (~3-7h after init time).
