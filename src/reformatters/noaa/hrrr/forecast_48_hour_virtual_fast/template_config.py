...
# Deaccumulation spans lead times, so a read-time codec cannot produce the materialized
# dataset's rates; the raw source quantities stand in under names describing what they are.
...
class NoaaHrrrForecast48HourVirtualFastTemplateConfig(
    ...
    noaa-hrrr-forecast-48-hour variable set. Every variable is single-level, so only
    wrfsfc source files are read."""
