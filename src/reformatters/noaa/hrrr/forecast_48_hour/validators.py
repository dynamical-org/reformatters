...
class CheckForecastCompleteness(validation.Validator):
    """
    ...
    def check(
        ...
        recent_ds = context.ds.isel(init_time=slice(-4, None))
        ...
        for init_time in recent_ds["init_time"].values:
            ...
            for lead_time in sorted(actual_lead_times, reverse=True):
                ...
                if not all(
                    da.isnull().all().item() for da in lead_data.data_vars.values()
                ):
