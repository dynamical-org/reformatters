...
# -----------------------------------------------------------------------------
# copy/paste all constants from the old template_config.py
# -----------------------------------------------------------------------------
...
VAR_DIMS: tuple[Dim, ...] = ("init_time", "lead_time", "latitude", "longitude")
APPEND_DIMENSION: AppendDim = "init_time"
...
VAR_SHARDS: dict[Dim, int] = {
    "init_time": 1,
    "lead_time": VAR_CHUNKS["lead_time"] * 2,
...
COORDINATES: Sequence[Coordinate] = (
    Coordinate(
        ...
        encoding=Encoding(
            ...
            chunks=int(pd.Timedelta(days=365 * 15) / INIT_TIME_FREQUENCY),
            shards=int(pd.Timedelta(days=365 * 15) / INIT_TIME_FREQUENCY),
        ),
...
DATA_VARIABLES: Sequence[NOAADataVar] = (
    NOAADataVar(
        ...
        encoding=Encoding(
            dtype="float32",
            fill_value=np.nan,
            chunks=tuple(VAR_CHUNKS[d] for d in VAR_DIMS),
            shards=tuple(VAR_SHARDS[d] for d in VAR_DIMS),
...
class GFSTemplateConfig(TemplateConfig):
    # raw inputs
    dataset_attributes: DatasetAttributes = DATASET_ATTRIBUTES
    time_start: pd.Timestamp = INIT_TIME_START
    time_frequency: pd.Timedelta = INIT_TIME_FREQUENCY

    dims: tuple[Dim, ...] = VAR_DIMS
    append_dim: AppendDim = APPEND_DIMENSION

    var_chunks: dict[Dim, int] = VAR_CHUNKS
    var_shards: dict[Dim, int] = VAR_SHARDS

    coords: Sequence[Coordinate] = COORDINATES

    # override computed data_vars
    ...
    def dimension_coordinates(self) -> dict[str, Any]:
        return _get_template_dim_coords(self.time_start + self.time_frequency)
