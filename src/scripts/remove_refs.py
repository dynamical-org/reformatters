...
Reads back what it deleted before committing, and is a dry run unless given --apply.
    ...
    # Specific append-dim positions, e.g. hours a re-backfill could not rewrite.
    DYNAMICAL_ENV=prod uv run src/scripts/remove_refs.py noaa-hrrr-analysis-virtual \
        --array dew_point_temperature_2m --at 2014-10-02T13:00 --at 2014-10-03T02:00
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="append",
        dest="at",
        help="Delete only this append-dim position (repeatable).",
    )
    ...
    whole_array = args.lead_index is None and args.before is None and not args.at
    ...
    for path in args.arrays:
        ...
        keys = (
            ...
            if whole_array
                ...
                _chunk_keys(
                    path, array, positions, args.lead_index, args.before, args.at
                )
            )
        )
        plan[path] = keys
...
def _chunk_keys(
    ...
    append_indices: Iterable[int] = range(
        len(positions) if before is None else int(positions.searchsorted(before))
    )
    if at:
        wanted = positions.get_indexer(pd.DatetimeIndex(at))
        assert (wanted >= 0).all(), (
            f"{path}: positions not in the record: {[t for t, i in zip(at, wanted, strict=True) if i < 0]}"
        )
        append_indices = sorted(set(wanted.tolist()) & set(append_indices))
