...
On a large ensemble archive also set `--probe-workers` (see Options): the scan's default probe fan-out is tuned for archives with far fewer source files per region job, and on an ensemble archive it both runs slower and holds several GB more.

Pass `--checkpoint-dir` on any whole-archive scan you cannot afford to repeat. The scan is the long pole of a virtual `run-all` — hours on a large ensemble archive — and without it an interruption anywhere loses all of it. With it the scan is written in slices as it completes and a re-run resumes from the last finished slice.
...
- `--checkpoint-dir <path>` Virtual stores: write the whole-archive manifest scan there in slices as it runs, and reuse any slice already present. An interrupted scan then resumes instead of starting over — worth setting on any archive whose scan runs for hours. Slices are keyed by dataset id, window and variable filter, so one directory can hold several datasets' scans; delete a dataset's files to force a fresh scan after a backfill changes the store.
- `--probe-workers <n>` Virtual stores: how many region jobs the whole-archive manifest scan probes at once (default: the tuned value). The scan's in-flight jobs spread across that many append-dim manifest splits at a time, so on an archive with many source files per job the working set outgrows the ref cache and manifests are refetched instead of reused. Lowering it then *raises* throughput and cuts peak memory. Measured on the 268-variable GEFS 16 day 0.5 degree virtual store (31 members x 105 leads x 2 file types = 6,510 source files per job): 8 workers ran at 25.8 region jobs/min with a 1.6 GB peak against 15.4 jobs/min and 7.2 GB at the 48-worker default. An archive with few files per job (the HRRR virtual store is ~100) needs no change.
