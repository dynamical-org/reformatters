...
When such a variable has no co-ingested co-member to stand in for it (it is alone in its source-file group, as in a single-variable dataset), the scan falls back to the variable's own nulls at the two run points and the summary's availability line says so. The reported gaps are then only as good as the points: pick two where the variable's semantic missing state does not apply — for a land-only product, two points solidly on land — or the scan reads the missing state as an outage.
