...
- **`value_timeseries_<var>.png`** — the variable's value over the **entire** dataset time range at two spatial points: per-timestep mean (line) with a ±1σ shaded band over the non-time dims (lead time, ensemble member). Reads the same point data as the nulls check (no extra read). Unlike `temporal_<var>.png` (a short window vs the reference), this spans all time and is meant to surface slow drifts and sudden discontinuities — e.g. a source that begins emitting a variable in different units shows up as a sharp step change in the mean line; a distribution change shows up as a step change in the band width. For analysis datasets (a single value per timestep) the band collapses and the plot is the raw value line.
...
- [ ] **No step change in the band width.** The ±1σ band reflects the spread across lead time / ensemble members at each timestep. An abrupt widening or narrowing that persists indicates a distribution change in the source (e.g. a change in precision, smoothing, or member generation). For analysis datasets the band is zero-width and only the mean-line check applies.
