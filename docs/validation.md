...
Where nothing was co-ingested — a single-variable dataset, or a variable alone in its source file group — availability falls back to the variable's own values at the two run points, counting a position unavailable only where it is NaN at **every** run point. Run points NaN at every position (an ocean point for a land-only variable) carry no signal and are dropped. This is coarser than a plain null scan: a position written over only part of the grid still reads as available. If no run point carries the variable, availability is reported as not measured, and no heatmap or per-variable availability plot is written — pin `--point` to cells that do carry it and re-run.
