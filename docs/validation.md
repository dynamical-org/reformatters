...
1. Open `availability_heatmap.png` first for the all-variable availability overview (era boundaries, shared gaps). It is the only all-variable image; compare variables against each other by opening several per-variable PNGs of the same type together (step 2).
