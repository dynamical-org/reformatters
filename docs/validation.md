...
- **For unexpected unavailable timesteps, you must fetch at least one of the affected source files from the upstream archive before attributing the gap to an upstream cause** — outages happen, but so do ingestion errors, and an LLM or a hurried reviewer will tend to default to "outage" without checking. Verify both the source data file (e.g. the GRIB) **and** any sidecar the reformatter depends on (e.g. the `.idx` byte index for GRIB-based datasets). Compare what you find against what the reformatted archive has at the same timestamp to determine whether to retry the backfill (`retry-filter:` line in `unavailable_timestamps.txt`) or document the gap as a confirmed source outage with the URL(s) you checked.
...
**Verification gate on `### Review notes`.** Any entry that attributes a gap, sentinel, or anomaly to an **upstream cause** (source outage, upstream archive gap, model physics, source-GRIB precision) is a load-bearing claim and must be backed by direct evidence: a fetched source file (per the unavailable-timestamp tactic in [3d](#3d-dig-into-each-follow-up-item)), an inspection of the reference dataset at the same timestamps, or a link to an authoritative third-party source. If you cannot produce that evidence, the item stays in `### For further review`, not `### Review notes` — "looks like an outage" without verification is exactly the failure mode this gate prevents.
...
- [ ] **Any non-zero null fraction you intend to label as an upstream outage has been verified by fetching at least one affected source file from the upstream archive.** If the file (and its sidecar index, if applicable) is present and intact, the gap is an ingestion bug, not an outage — keep it in `### For further review` and use the `retry-filter:` line in `unavailable_timestamps.txt`.
