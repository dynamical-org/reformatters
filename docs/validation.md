...
- **`value-timeseries` is sampled.** Instead of reducing mean ± std over lead × member at every position (which decodes the whole column), it strides the append dimension and decodes **one message per sampled position**, rotating through lead/member/level with position so the series samples every non-spatial dim across the archive. A chunk is a full spatial field, so both run points are read from the same decode. This still surfaces drift and unit step-changes (per-lead/level texture is expected noise); per-variable loads run a few ahead of the plotting loop. The summary marks the series "sampled".
...
Budget accordingly: a full `run-all` on a virtual store decodes roughly 200 messages per variable for the sampled value time series plus one series per point for the temporal comparison — order 10⁴–10⁵ full-field messages for a ~150-variable dataset. Each variable's decode count is logged before its read starts. Bound a run with `--variable`/`-v` or `--start-date`/`--end-date`.
