...
- **Remove data** — no backfill operation does this. Metadata is copied from the template into the store rather than synced with it, and writes only ever set chunks, so an array the template no longer declares and chunks a variable should no longer carry both survive every backfill. `DYNAMICAL_ENV=prod uv run main <dataset-id> remove-chunks <array>... [--lead-index N] [--before <timestamp>] [--at <timestamp>...] | --whole-array` deletes them from the primary store and every replica, one commit per array; a deleted chunk reads as the array's fill value. It is a dry run without `--apply`, and after each commit reads back that what it deleted is gone. Split a selection spanning many arrays and positions with `--before`/`--at` rather than holding millions of deletes in one commit. Removal is a manual one-off, never a step of a backfill or an update — both only ever write, and an update that re-ingests the removed positions (its `operational_update_window` still covers them) puts them straight back. Run it between update fires. A selection must cover whole shards, so on a sharded (materialized) array `--before`/`--at` only work at shard boundaries and `--lead-index` needs one lead per shard.
