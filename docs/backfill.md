...
- **Remove data** — no backfill operation does this. Metadata is copied from the template into the store rather than synced with it, and the virtual write path only ever sets refs, so an array the template no longer declares and chunks a variable should no longer carry both survive every backfill. `uv run src/scripts/remove_refs.py <dataset-id> --array <name> [--lead-index N | --before <timestamp> | --at <timestamp> ...]` deletes them; a deleted chunk reads as the array's fill value. It is a dry run without `--apply`, and needs credentials that can read the storage secret.
