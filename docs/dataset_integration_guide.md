...
Parallelism: Set `workers_total` and `parallelism` on the `ReformatCronJob` using `self.update_num_variable_groups()`. Multiply by 2 if `operational_update_jobs` reprocesses the most recent time slice (see GEFS datasets for examples).
