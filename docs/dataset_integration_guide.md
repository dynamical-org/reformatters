...
- **Shared provider utilities** — check `src/reformatters/<provider>/` for shared modules (e.g., `ecmwf/ecmwf_grib_index.py`, `noaa/noaa_utils.py`). Use these rather than writing your own.
- **Variable names and metadata** — cross-reference variables that already exist in other datasets. If another dataset already has an equivalent variable (e.g. `temperature_2m`), match those names and metadata exactly. Run `uv run pytest tests/common/datasets_cf_compliance_test.py -x` to catch naming inconsistencies early.
...
Follow conventions in `__main__.py` to register your dataset with primary and replica storage configs.
