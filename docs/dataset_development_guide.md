...
Present the exploration findings and settle the scope with the human. Always align on the exact **provider, model, and variant(s)** to produce — including whether this run creates several related datasets (see "Multiple datasets in one run" above) — and on the **variable set** for each (usually all available variables for a virtual dataset, a common subset for a materialized one). Beyond that, raise the non-obvious questions that surfaced while mapping the source data onto a datacube in our conventions — the decisions the exploration can't settle on its own (an awkwardly-structured or intermittently-available variable, an irregular level set, a coordinate that doesn't map cleanly, a choice between combining or splitting sources). Use judgement about what's worth asking rather than running a fixed checklist. In add-variable mode this collapses to confirming the variable's name, level, and attrs. Record what's decided — it drives every later stage.
...
- If the dataset already has an active operational update cronjob, do not suspend it — overwrite backfills run concurrently with updates, deferring the region the updates rewrite to them (see "Concurrency with operational updates" in [backfill.md](backfill.md)).
- New dataset: its cron jobs ship suspended (`suspend=True`). Once this backfill completes, open a PR removing that so operational updates start keeping the store current — the targeted re-backfills in the Validate stage then run concurrently with the updates.
