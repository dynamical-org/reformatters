...
- **Clean up a killed or failed job**: After confirming its work is no longer needed, run `DYNAMICAL_ENV=prod uv run src/scripts/cleanup_job_artifacts.py <dataset-id>` and review the dry-run plan, including branch snapshots and coordination-object counts; then repeat with `--apply`. Pass repeatable `--job <kubernetes-job-name>` options to limit the cleanup. The script deletes artifacts only for absent, Complete, or Failed jobs and refuses the entire apply if any selected job is still active.
