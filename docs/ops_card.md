...
- **`noaa-hrrr-forecast-18-hour-virtual-fast-validate` fails on `CheckMirrorWindow`**: if the message says that `init_time` starts too early or the window contains too many positions, the update is not moving the window. Fix the update and let the next scheduled fire drop the expired positions. If the oldest referenced init does not decode, check `noaa-hrrr-nomads-mirror-gribs` and the R2 public domain. Files NOMADS has rotated away are unrecoverable in this product; use `noaa-hrrr-forecast-18-hour-virtual` for those forecasts.
- **`noaa-hrrr-forecast-18-hour-virtual-fast-update` reports that the store is not aligned with the template**: another writer moved the window, or a manual run used a different fire time and therefore generated a different window start. Ensure only one update writer is active and let the next scheduled fire run against its matching template.
