...
- **Clean up a killed or failed job**: A job that does not finish leaves behind its `_job_<job name>` icechunk branch and `_internal/<job name>/` coordination files. Run `DYNAMICAL_ENV=prod uv run src/scripts/cleanup_job_artifacts.py <dataset-id>` and read the dry-run plan: a branch's `not-on-main` count is the commits that never reached `main`, which deleting it discards. Repeat with `--apply` to delete, and pass repeatable `--job <kubernetes-job-name>` options to limit which jobs are touched. Artifacts are deleted only for absent, Complete, or Failed kubernetes jobs, and the apply refuses outright if any selected job is still active. Deleting a branch unpins the manifests it held but frees no storage; `src/scripts/icechunk_utils.py garbage-collect` does that.
