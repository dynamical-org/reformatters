...
- **CHIRPS source day permanently missing** (`CheckCurrentData` fails and the update's newest published day stops advancing while later days exist at the source): Confirm the day returns 404 at the source, add it to that product's `known_missing_days`, merge and deploy, then re-run update and validation. Remove the day from `known_missing_days` once the source publishes it. If updates no longer replay the day's shard, fill it with an `overwrite-chunks` backfill targeting that shard. Record historical dates in the validation report's Review notes, not variable metadata.
