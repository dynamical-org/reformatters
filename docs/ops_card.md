...
- **`noaa-hrrr-forecast-18-hour-virtual` reads broken refs or the mirror cron misbehaves**: the NOMADS mirror bucket ages files out after three days, so a file NODD never published keeps only mirror refs until then; nothing to repair on our side unless NODD is missing it for good. Suspending `noaa-hrrr-nomads-mirror-gribs` stops new copies; the update then takes every file from NODD, and files already in the mirror are still rewritten from NODD by later fires.
