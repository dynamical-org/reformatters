...
- **Data that should not be in a store** (a stale hour-0 field, an array the template dropped): no update or backfill removes anything. Use `uv run main <dataset-id> remove-chunks` as a manual one-off between update fires — see "Remove data" in [docs/backfill.md](backfill.md).
