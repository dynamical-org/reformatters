...
NOMADS publishes each HRRR file 60 to 80 s before NOAA's AWS archive (NODD) lists it, and NODD occasionally holds a file for ten minutes. `noaa-hrrr-forecast-18-hour-virtual-fast` is the 18-hour virtual product (same variables, groups, chunking and encodings as `noaa-hrrr-forecast-18-hour-virtual`, a recent `append_dim_start`) fed from a rolling mirror of NOMADS in Cloudflare R2, with NODD as the floor and the durable home of every ref. Three single-pod crons:

- **The mirror** (`noaa-hrrr-nomads-mirror-gribs`, an `OperationalResources` cron in `noaa/hrrr/nomads_mirror.py`, not a dataset) fires hourly, waits for the init's publication window, then polls NOMADS once a second with one directory listing and copies each of the init's files, sfc, prs and nat, f00 to f18, GRIB and NOAA's own `.idx`, into the R2 bucket `noaa-hrrr-nomads-mirror` under the exact key NODD will use. Every listing and copy goes through the shared `nomads_rate_limiter` (100 per minute, under NOMADS' 120 per minute ban threshold): about 60 listings plus a few copies per minute. A data file is copied only once it parses as whole GRIB2 messages, and its index only once it lists exactly the messages the mirrored data file holds (the data file is fetched again and replaced if NOMADS appended messages after the first copy), so an index in the mirror describes the data file beside it. Whether NOMADS ever lists a file before it is fully written is not documented; a data file and index that were both partial yet agreed would be copied as they are, which is why the update ingests no file whose index lacks a message the template expects of it.
- **The update** is the dataset's ordinary virtual update with overrides on `NoaaHrrrForecast18HourVirtualFastRegionJob`. `discover_available` takes a pending file from NODD when NODD has it and from the mirror otherwise; refs into the mirror point at its public domain (`https://noaa-hrrr-nomads-mirror.r2.dynamical.org/`), a second virtual chunk container, and differ from NODD refs only by prefix. Before a batch with mirror-sourced files is committed, the job writes one **pending-repoint record** per such file (an empty coordination file under the dataset's `_internal/pending-repoint/`, named by the file's NODD key). On every later update fire the recorded files are re-offered, NODD-only, probed every 60th discovery sweep so they never sit ahead of the current init's files, and at most `max_repoints_per_fire` oldest first; once NODD supplies a file its refs are rewritten from NODD and the record is deleted after that commit. The record, not the mirror's listing, is what drives the rewrite, so a file whose mirror copy has already expired is still rewritten as soon as NODD publishes it. A file whose index lacks any message the template expects, whether a partial mirror pair or a truncated NODD upload, is skipped and retried on later fires rather than ingested in part. A mirror that cannot be listed leaves NODD as the floor for new files. Backfills use NODD only.
- **Validate** is the standard virtual validation plus `CheckMirrorRefsRepointed`, which fails when a pending-repoint record's init is older than 36 hours: the mirror expires objects three days after copying them, and a read through a ref whose object is gone raises rather than returning the fill value. A file NODD publishes late is rewritten by the next update fire after it lands, with no manual step. A file NODD never publishes keeps its record and keeps validation red until someone removes that file's refs from the store and its record from `_internal/pending-repoint/` by hand (see the ops card); this PR ships no tool for that.

Repointing writes a new snapshot; older snapshots keep their mirror refs and lose those chunks when the mirror expires them. A reader of this product must reopen `main` rather than hold a snapshot, and its catalog description must say so.

Readers see mirror-pointed chunks (typically until the next hourly fire) only if they authorize the mirror container, an `Http` container that takes `icechunk.Credentials.HttpAccess()` rather than S3 credentials; the STAC catalog item must list both container prefixes.
