...
3. `file_refs(coord, file_size) -> [VirtualRef]` — given one available file, return every pointer it contributes — a `VirtualRef` is `(source location, byte offset, length) -> one output chunk`. Resolve byte ranges however the source allows: parse a sidecar index, scan the data file, or (one message per file) point at the whole file. Refs are in coordinate-label space; the chunk index is resolved centrally. Raise `SourceFileRejectedError` only after establishing that the published contents of this individual file are unsafe to reference. Returning no refs or raising any other exception fails the worker so a code, storage, or transient download failure cannot become missing data.
...
**Optional overrides (working defaults):** `filter_already_present` (manifest probe), `representative_var` / `representative_probe_loc` (the variable and cell standing for the whole file — first instant var among the coord's own `data_vars`, at `out_loc` plus the first label of each dim it does not specify; only override for a packing those rules miss), `source_file_anomalies` (observe accepted source results before the empty-ref invariant is checked), the `tick_interval` (1s) / `download_concurrency` (64) class attrs, and `process_virtual_refs` itself (only for a fundamentally different batching policy).

**Source rejection policy.** The loop logs each `SourceFileRejectedError` at WARNING, discards that file, and emits one ERROR aggregate for the worker grouped by reason. A discarded coord is not retried within that run. Because no representative ref is written, `filter_already_present` automatically re-offers it on later runs, so the gap self-heals if the source republishes a valid object. Operational manifest-completeness validation reports the gap on every run while its coordinate remains in the recent window. Historical backfill gaps lie outside that validation window and have no automatic detector on `main`; availability plots or a separate historical manifest scan must expose them. The two known truncated HRRR objects are the exception to this general protocol: their discovery denylist withholds them before `file_refs`, so they do not enter rejection accounting.

Operational updates do not apply a rejection-rate breaker: their source populations are small enough that a couple of bad objects could suppress the whole active window. Their per-file WARNINGs, worker ERROR aggregate, and completeness validation remain active.

For backfills, one source-file rejection is always tolerated. Beyond one, a rejection rate of at least 10% of the worker's initially planned files or more than 20 total rejections reports a systemic source failure. The denominator includes every file in the worker's initial post-manifest-filter population. The worker still commits every good ref and, in a multi-worker run, writes its results file; the last worker publishes the completed branch, then reports a non-zero job outcome. This ordering prevents a source-rejection burst from stranding finalization.

Missing representative coverage and GEFS missing-variable completeness remain fatal internal assertions: either can mean a catalog or representative bug, and discarding the whole file would also discard its otherwise valid variables. The representative probe is an ingestion-presence marker, not a source-file completeness or truncation detector. A partial file containing the probe is accepted and then no longer re-offered. Products vulnerable to matching short data and index objects need an independent ingest-time completeness signal, such as an object-size or index-line-count check, before partial refs are committed.
