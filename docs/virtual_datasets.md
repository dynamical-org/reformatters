...
A moving-window dataset must store one append-dimension position per data chunk, use a single Icechunk repository without replicas, and generate a template whose `append_dim_coordinates` begin at the desired window start. Every write batch checks alignment against its own writable session and commits without rebasing. A `ConflictDetector` rebase would replay positional references unchanged over another writer's drop and place them at the wrong labels; instead, a stale writer fails its alignment check or commit. Validation also checks alignment before any manifest probe.
...
Window retention must stay below the source's object expiry by more than the longest update outage the system is intended to ride out. `CheckMirrorWindow` enforces this contract for `noaa-hrrr-forecast-18-hour-virtual-fast`: it alerts if the window stops moving or a chunk of the oldest referenced init no longer decodes over the public domain. This product's first sweep of each hourly fire offers every missing mirror file in its 72-hour window; later sweeps poll only the most recent six hours. A new store is populated by one mirror-fed `create-new-store` backfill, which fills whatever files the mirror still holds.
...
- **Validate** runs the standard virtual checks plus `CheckMirrorWindow`. The check verifies that `init_time` continues to advance and that the oldest referenced init still decodes. Manifest completeness is held over the most recent six hours only, so a file NOMADS never published alerts for hours rather than for the life of the window.
