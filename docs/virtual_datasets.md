...
2. `unfinished_work` (by default `filter_already_present`) drops files whose refs are already in the manifest; the survivors across the worker's jobs are unioned into `remaining`. An `overwrite_chunks` backfill skips this step, so refs pointing at the wrong source file are rewritten rather than treated as done. A dataset overrides `unfinished_work` to add work the manifest probe cannot see, such as an ingested file whose refs must be rewritten from another source (see [NOMADS cache](#nomads-cache-a-second-source-ahead-of-nodd)).
...
4. Each yield is committed atomically: open fresh writable sessions (a committed icechunk session is read-only), grow the append dim if needed (`sync_dims_to`), `set_virtual_refs`, commit. Once the commit has landed, `committed(batch)` may return follow-up coords, which join the pending work; a failed commit never reaches it.
...
## NOMADS cache: a second source ahead of NODD

NOMADS publishes each HRRR file 60 to 80 s before NOAA's AWS archive (NODD) lists it, and NODD occasionally holds a file for ten minutes. The `noaa-hrrr-forecast-18-hour-virtual` product closes that gap with two processes:

- **The mirror** (`noaa-hrrr-nomads-cache`, an `OperationalResources` cron in `noaa/hrrr/nomads_mirror/`, not a dataset) fires hourly, waits for the init's publication window, and copies the surface files f00 to f18 from NOMADS into the cache bucket `s3://dynamical-noaa-hrrr-nomads/` under NODD's exact keys. Every NOMADS request goes through the shared `nomads_rate_limiter`; the download itself is the probe (a 404 means not yet published), one transfer at a time, no directory listings, and never NOMADS' `.idx` sidecar, which can trail its GRIB by minutes. The mirror validates each file (section-0 tiling, `7777` trailers, a `wgrib2 -s` inventory whose offsets match, a message count no lower than the previous lead's), then uploads the GRIB before its index so an index in the cache implies a complete data object. wgrib2 is built from a pinned source release in the Docker image because NOAA makes its sidecars with it and the ref builder matches on those exact inventory strings.
- **The ingest** is the dataset's ordinary update. Its region job registers the cache as a second virtual chunk container and, in update mode, discovers each pending file on the cache first (data plus index, and the index must cover every variable the coord carries, else the coord is routed to NODD for the rest of the fire) and on NODD otherwise. A file ingested from the cache is committed like any other; `committed` then schedules a *repoint twin* for it, a coord for the same file that is probed on NODD only and rewrites the refs from NODD's `.idx` a tick after NODD publishes. When the twin's commit lands, `committed` writes a `<key>.repointed` marker beside the cache object and tags the GRIB and index `repointed=true`; the bucket lifecycle expires only tagged objects, so `main` never references an expired object and a file NODD never publishes stays cached. `unfinished_work` merges the update window's candidates with the cache listing by file, so a repoint interrupted by a pod replacement or a late NODD is retried on every fire. Backfills use NODD only. `NoaaHrrrForecast18HourVirtualRegionJob.cache_first = False` is the rollback knob: new files come from NODD while repoints, markers and the container registration keep working.

Readers see the cache-pointed window (typically about a minute per file) only if they authorize the cache container; the STAC catalog lists both containers. A snapshot taken before a repoint keeps reading the cache object until the lifecycle expires it. `CheckNomadsCacheRepointed` fails validation when a cached file older than three hours has no marker, i.e. NODD has not published it and `main` still depends on the cache for it.
