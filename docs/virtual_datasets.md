...
- **Total manifest count `M`** — `M = array_count × ceil(appends / split_size)`. Every commit re-serializes the snapshot's manifest list (`O(M)`, plus an `O(M²)` lookup in unpatched icechunk — fixed by our fork's O(1) index) and the snapshot object grows with `M`. Smaller split → more manifests → larger `M`. For a many-variable dataset `M` is dominated by `array_count`, so over-splitting the low-ref-count arrays is the biggest, least useful contributor.
