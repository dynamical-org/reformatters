...
**Files the source hasn't published yet roll to the next fire rather than failing the run.** An update stops polling at its `poll_deadline` — the scheduled fire plus the cron job's `pod_active_deadline`, less a small grace to exit and check in — and exits successfully with those files still pending; the next fire re-sweeps the same window and ingests them a tick after they publish, so waiting in-run and handing off cost the same latency. A tick runs to completion once started, so the loop stops as soon as another tick as long as the longest one so far would not fit before the deadline; otherwise the pod is killed mid-tick at its active deadline and its cron check-in never lands, reporting a clean roll-over as a monitoring timeout. `pod_active_deadline` therefore sizes how long a fire chases its own files: cover the source's typical publication time with headroom, and let a late cycle roll over rather than stretching past the fire interval (`concurrencyPolicy: Replace` kills a run that outlives its successor's fire anyway). The deadline is anchored to the scheduled fire, not to process start, so a pod replacing an evicted one stops when the pod it replaced would have instead of polling into the next fire. A source that stops publishing altogether is caught by the validation cron (data currency and manifest completeness), not by the update job.
