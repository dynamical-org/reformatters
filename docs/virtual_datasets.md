...
2. `discover_available(pending) -> [(coord, file_size)]` — given the source files not yet ingested, return those available to fetch right now, each with its data-file size. Obstore-listable backends one-line this via `discover_available_by_obstore_listing` (below); other sources implement it directly. Withholding a file that *is* available defers it — it stays pending and is offered again next tick — which is how a dataset keeps files that have to land together in one commit. `ingested_through`, the newest append-dim label the store covered when the run started, separates a file that fills in an existing position from one that extends the array. By convention a forecast dataset withholds nothing: the newest init time is normally partial. An analysis dataset built from several lead times holds leads beyond lead-time zero until that position's lead-time-zero file lands, so the leading edge is never ragged (`noaa/hrrr/analysis_virtual`). A forecast dataset virtual over a Zarr source is the exception: it waits for all of a position's source chunks.
...
- `out_loc() -> {dim: label}` — the output region the file fills: a label per dim the file pins, omitting any dim it spans wholly. The inherited default works when the coord's fields are all dim names; override otherwise.
...
**Optional overrides (working defaults):** `filter_already_present` (manifest probe), `representative_var` / `representative_probe_loc` (the variable and cell standing for the whole file — first instant var among the coord's own `data_vars`, at `out_loc` plus the first label of each dim it does not specify; only override for a packing those rules miss), the `tick_interval` (1s) / `download_concurrency` (64) class attrs, and `process_virtual_refs` itself (only for a fundamentally different batching policy).
...
**NOAA GRIB sources** (`noaa/noaa_virtual_region_job.py`): `NoaaVirtualRegionJob` implements `discover_available` and `file_refs` for any NODD bucket whose GRIB files carry a `.idx` sidecar — a subclass supplies the `source_location_prefix` / `source_bucket_region` class attributes, a `NoaaVirtualSourceFileCoord[<its DataVar>]` subclass, `generate_source_file_coords`, and `operational_update_window`. It may also override `_check_refs_complete`, called once a file's refs are built, to reject a file whose index did not carry every variable the coord asked for. A root variable's `grib_index_level` is the literal idx level string; a vertical-group variable's is a format string filled per level, written `{level:g}` so an integer and a float coordinate both render as the source index spells them (`1000`, `0.01`). A sidecar index can go stale against a re-uploaded object while still naming in-bounds byte ranges, so `file_refs` checks its last offset against the data file's own GRIB header — one extra 16-byte ranged GET per file — and discards the whole file unless a fitting edition 2 message starts there. The last offset is where staleness shows, since a message that changed size shifts every offset after it; the check requires the message to fit rather than to end at EOF, because a healthy index may list fewer messages than the object holds.
...
**Files the source hasn't published yet roll to the next fire rather than failing the run.** An update stops polling at its `poll_deadline` — the scheduled fire plus the cron job's `pod_active_deadline`, less a small grace — and exits successfully with those files still pending; the next fire re-sweeps the same window and ingests them a tick after they publish, so waiting in-run and handing off cost the same latency. `pod_active_deadline` therefore sizes how long a fire chases its own files: cover the source's typical publication time with headroom, and let a late cycle roll over rather than stretching past the fire interval (`concurrencyPolicy: Replace` kills a run that outlives its successor's fire anyway). Both ends of a fire's work are anchored to its scheduled fire rather than to process start: a pod replacing an evicted one stops polling when the pod it replaced would have, and its window ends at that fire, so it finishes its predecessor's steps instead of adopting the next fire's and waiting out its deadline on files that publish after it. A source that stops publishing altogether is caught by the validation cron (data currency and manifest completeness), not by the update job.
...
Icechunk stores each array's chunk references in one or more immutable manifest objects, split along a dimension via `IcechunkVirtualConfig.manifest_split` (built with `manifest_append_dim_split`). A commit rewrites only the split(s) whose refs changed: an operational append lands in the current (highest-index) split of each touched array while historical splits carry over unchanged, so operational commit cost is the active splits' size, not the archive's. Three costs pull against each other when sizing splits:

- **Operational commit latency** — rewriting a split is a full read-modify-write of every ref already in it, so cost tracks **refs per commit**: the refs in every manifest the commit rewrites. For an operational append, this is arrays touched × refs per active split, not the refs being added. Order 10⁷ refs per commit is the scale at which commit latency reaches a few seconds; fewer arrays tolerate more refs per split.
- **Manifest size (bytes)** — a reader downloads a whole manifest to resolve *any* chunk in it. Keep a full manifest within a reader-friendly size: rough budgets are ≤ 3 MiB per single-level variable (a small web app plotting one field pays one manifest + one chunk) and 5–8 MiB for vertical-group variables (those readers expect bulk downloads). Smaller splits mean a whole-history reader fetches more objects for the same total bytes — fine at reader concurrency, but don't shrink below ~1000 refs/manifest (see gotchas below).
- **Total manifest count `M`** — `M = array_count × ceil(appends / split_size)`. Every commit re-serializes the snapshot's manifest list (`O(M)`) and the snapshot object grows with `M`. Smaller split → more manifests → larger `M`. For a many-variable dataset `M` is dominated by `array_count`, so over-splitting the low-ref-count arrays is the biggest, least useful contributor.

Subject to the reader size budgets above, priority is operational latency, then avoiding many tiny manifest objects (they slow listings and multiply reader requests), then backfill CAS contention.

Changing `split_size` on an existing archive is safe and incremental: on each array's next commit, flush re-windows that array's existing manifests to the new boundaries (windows with no changes but partial overlap are rewritten). The first operational commit after a shrink therefore re-windows every touched array's manifests — including full historical ones — in one commit: bounded, one-time, minutes-scale for a large archive; plan for it on the first fire after deploy.
...
To convert a size budget into a `split_size` you need **bytes per ref**. Expect roughly **16 bytes/ref**, but location compressibility varies with URL structure, so **check it on each new dataset** once some data is written: `repo.list_manifest_files(snapshot_id)` returns every live manifest's exact `size_bytes` and `num_chunk_refs`. Don't estimate from S3 object sizes (partially-filled splits skew low, and you can't tell fullness from the outside). Two gotchas: the ~1000-ref floor on a group's split is about not creating many tiny objects, not compression — icechunk trains its location dictionary only at ≥ `min_num_chunks` refs per manifest (default 1000), and a manifest below that is no larger per ref; and refs are only this small when a `VirtualChunkContainer` prefix covers the locations.
...
The `-validate` CronJob runs after each operational update. A dataset lists every check — configured `validation.Validator` instances — in its `validators()`. `validate_dataset` runs each against a `ValidationContext` (the opened dataset, store, append dim, template variables). A check that probes the manifest sets `requires_virtual_dataset`, and the context additionally carries the operational-window region job, built once (via `operational_update_jobs`) and shared across primary + replica.

Two virtual checks ship in `validation.py`; both are bounded so the whole validation stays well under a couple of minutes even on a large ensemble dataset:

- **`CheckVirtualManifestCompleteness`** — the virtual analog of the materialized `CheckExpectedShards`. Re-runs the operational filter (`region_job.source_file_coords()` + `filter_already_present`) over the recent window and checks, per append-dim position, the fraction of expected source files present against `min_present_fraction` (a tuple indexed newest-first; older positions held to its last value). Positions past the store's append-dim extent are skipped, since the window runs ahead of what the source has published: a wholly un-ingested recent stretch is `CheckCurrentData`'s job, while an interior gap still fails here. `(1.0,)` requires every remaining append-dim position fully ingested; `(0.5, 1.0)` lets the newest be half-published (e.g. GEFS 35-day's slow long lead times) while older ones must be complete. The last tier must be 1.0 (asserted) since it holds for every older append-dim position; loosen only the leading tiers, which cover positions the source may still be filling in. Fails — never silently passes — if the window is shorter than the threshold tiers. Reusing the dataset's own coord generation means structural absences (e.g. hour-0 accumulated vars) are already excluded — no false positives. Cheap: one ref-existence probe per file, no decode. `include_vars` / `exclude_vars` narrow the check to the source files carrying those variables, so variables publishing on different schedules can be checked separately, each held to a whole 1.0 from the position where its own files are expected; the split must fall on source-file boundaries (asserted), since presence is probed through each file's representative variable.
- **`CheckVirtualDecodeHealth`** — where completeness checks references *exist*, this checks the ones that exist actually decode. It keeps only the source files present in the manifest (`filter_already_present`) — so a not-yet-published ref is never mistaken for a decode failure — and decodes a bounded sample (first + last + interior lead times across every ensemble member). `positions` (an int, default 1) targets that many of the newest positions *with data*, so a broken newest reference is caught at the very next validation rather than a cycle later; use more than 1 when the newest position does not carry every variable. `positions="all"` covers the whole window, optionally capped to `max_positions` evenly spaced positions for an offline sweep. A variable fails if any sampled chunk errors or all of its sampled chunks decode entirely NaN; the check fails — never silently passes — when no references are present. `allow_all_nan_vars` explicitly names exceptional variables whose sampled chunks may legitimately all be NaN; they still fail on decode errors or missing sampled references. The full-archive NaN/coverage scan stays offline (one full-message decode per chunk across all leads × members is hours).
...
A virtual region job with `drops_before_template_start = True` keeps only the append-dimension span in its current template. At the start of every update fire, `drop_before_template_start` removes store positions before the template's first label in a commit of its own. `Session.shift_array` moves data chunk references within each manifest, coordinate arrays are rewritten, and every append-dimension array is shrunk. This restores the invariant that store position _i_ is template position _i_, so the rest of the virtual write loop uses its ordinary positional chunk keys unchanged.

A moving-window dataset must store one append-dimension position per data chunk, use a single Icechunk repository without replicas, and generate a template whose `append_dim_coordinates` begin at the desired window start. Every update write, the metadata refresh and each ref batch, checks alignment against its own writable session and commits without rebasing. A `ConflictDetector` rebase would replay positional references unchanged over another writer's drop and place them at the wrong labels; instead, a stale writer fails its alignment check or commit. Validation asserts the same alignment before any manifest probe, against the template of the update cron's most recent fire, so a fire whose drop did not run fails the validation job on that assertion rather than on a named check.

The coordinate extent is not the data extent: positions for source files that have not arrived contain fill values. For `noaa-hrrr-forecast-18-hour-virtual-fast`, the first `init_time` advances hourly. A reader of a source-backed moving window must also account for source expiry: an Icechunk snapshot can outlive the source objects it references, and reads from such a snapshot raise after those objects are deleted. Long-lived readers therefore reopen `main` to follow the window.

Window retention must stay below the source's object expiry by more than the longest update outage the system is intended to ride out. For `noaa-hrrr-forecast-18-hour-virtual-fast` the validation job's alignment assertion alerts when the window stops moving, and `CheckMirrorWindow` alerts when the oldest data in the window, one variable per file type, no longer decodes over the public domain. Each hourly fire of this product sweeps every position of its template once for missing mirror files, then polls only the most recent six hours, so a file that never arrives does not hold the run open. A new store is populated by one mirror-fed `create-new-store` backfill, which fills whatever files the mirror still holds; it is the only backfill this product supports. Pass it `--append-dim-end` equal to the update cron's most recent fire time (`HH:50`), so the store it creates is aligned with the template validation and the next update use.

## NOMADS mirror

NOMADS publishes each HRRR file 60 to 80 seconds before NOAA's AWS archive lists it. `noaa-hrrr-forecast-18-hour-virtual-fast` is a moving-window product fed exclusively from a rolling mirror of NOMADS in Cloudflare R2. The NODD-fed `noaa-hrrr-forecast-18-hour-virtual` product holds the history. Three single-pod crons operate the fast product:

- **The mirror** (`noaa-hrrr-nomads-mirror-gribs`, an `OperationalResources` cron in `noaa/hrrr/nomads_mirror.py`, not a dataset) fires hourly, waits for the init's publication window, then polls NOMADS once a second with one directory listing and copies each of the init's sfc, prs, and nat files from f00 to f18, both GRIB and NOAA's `.idx`, into the R2 bucket `noaa-hrrr-nomads-mirror`. Every listing and copy goes through the shared `nomads_rate_limiter` at 100 requests per minute, under NOMADS' 120-per-minute ban threshold. A data file is copied only once it parses as whole GRIB2 messages, and its index only once it lists exactly the messages that the mirrored data file holds. The data file is fetched again and replaced if NOMADS appended messages after the first copy. Whether NOMADS ever lists a file before it is fully written is not documented; a data file and index that were both partial yet agreed would be copied as they are, so the update rejects any file whose index lacks a message the template expects.
- **The update** reads only the mirror through its signed R2 API and writes references to the mirror's public domain, `https://noaa-hrrr-nomads-mirror.r2.dynamical.org/`. A mirror file that fails the completeness check is retried on later fires instead of being ingested in part. Each fire advances the window before offering missing files as described in [Moving window](#moving-window).
- **Validate** runs the standard virtual checks plus `CheckMirrorWindow`. The job asserts that the store's window matches the most recent update fire, and the check verifies that the oldest data in the window still decodes. Manifest completeness is held over the most recent six hours only, so a file NOMADS never published alerts for hours rather than for the life of the window.

Readers authorize the mirror container, an `Http` container that takes `icechunk.Credentials.HttpAccess()` rather than S3 credentials. The STAC catalog item lists this container prefix.

### R2 setup

Access follows the pattern of every S3-compatible store in this repo: a kubernetes secret holding `icechunk.s3_storage` keyword arguments. The mirror's secret is `noaa-hrrr-nomads-mirror-storage-options-key`:

```json
{"endpoint_url": "https://<account-id>.r2.cloudflarestorage.com", "region": "auto", "force_path_style": true, "access_key_id": "...", "secret_access_key": "..."}
```

R2's S3 API has no anonymous access, so the mirror cron and the update pod both mount it: the cron writes through it and the update lists the mirror and reads indexes and GRIB headers through it (`download.signed_s3_store`). Readers, including the validation job's decode check, fetch chunk bytes from the bucket's public custom domain, which is the container prefix. Bucket setup, none of it done by code:
...
2. Add an object lifecycle rule that deletes objects 4 days after upload, plus the rule that aborts incomplete multipart uploads after a day. The fast product keeps an init for up to 73 hours after its first file is copied (72 hours of retention, then the next hourly fire drops it), so the fourth day is the update outage the window rides out. Deletion runs on R2's schedule, up to a day after the expiry; that delay is not part of the margin.
