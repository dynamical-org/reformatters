...
For backfills, one source-file rejection is always tolerated. Beyond one, a rejection rate of at least 10% of the worker's initially planned files or more than 20 total rejections reports a systemic source failure. The denominator includes every file in the worker's initial post-manifest-filter population. The worker still commits every good ref and writes its results file; the last worker publishes the completed branch, then reports a non-zero job outcome. This ordering prevents a source-rejection burst from stranding finalization.
