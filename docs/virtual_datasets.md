...
Per-tick commit latency is dominated by the manifest read-modify-write of every array the tick's files touch (see [Manifest splitting](#manifest-splitting) for the cost model). Every icechunk repo is opened with `ManifestConfig.max_concurrent_manifest_fetches_during_commit` raised so the per-array fetches overlap.
...
- **Operational commit latency** — rewriting a split is a full read-modify-write of every ref already in it, paid by every array the commit touches, so cost tracks **refs per commit = arrays touched × refs per active split**, not the refs being added. Order 10⁷ refs per commit lands within the 2–3 s operational budget; fewer arrays tolerate more refs per split.
...
Priority: operational latency first, then avoiding many tiny manifest objects (they slow listings and multiply reader requests), then backfill CAS contention.
