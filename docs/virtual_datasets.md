...
The coordinate extent is not the data extent: positions for source files that have not arrived contain fill values. The first append-dimension label advances with every drop. When the source expires its objects, an Icechunk snapshot can outlive the objects it references, and reads from such a snapshot raise after those objects are deleted. Long-lived readers therefore reopen `main` to follow the window.

Window retention must stay below the source's object expiry by more than the longest update outage the system is intended to ride out, and a validator should decode the oldest data the window holds, since no other check reads that end of it. A new store is created by one `create-new-store` backfill whose `--append-dim-end` is the update cron's most recent fire time, so the store is aligned with the template that validation and the next update use; a moving-window store cannot be re-backfilled.
