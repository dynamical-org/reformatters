...
Per-tick commit latency is dominated by the manifest read-modify-write of every array the tick's files touch (see [Manifest splitting](#manifest-splitting) for the cost model). Every icechunk repo is opened with `ManifestConfig(max_concurrent_manifest_fetches_during_commit=16)` so the per-array round trips overlap; the CPU side is bounded by the active split size and the pod's cores.
...
- **Operational commit latency (CPU)** — rewriting a split is a full read-modify-write: flush fetches the active manifest, decodes every ref in it, merges the new refs, re-serializes and re-compresses. Measured on HRRR-spatial: **~0.1 s of single-threaded CPU per MiB of active manifest, per changed array, per commit** (zstd + flatbuffer rebuild, proportional to refs in the window — not to the refs being added). A many-array group multiplies this: 142 single-level arrays × ~1.7 MiB active windows ≈ 24 s of CPU per commit. For a dataset with an operational latency target, this bound picks the split size: `split_size × refs_per_index × 16.4 B` should stay in the low hundreds of KiB per array.
...
- **Total manifest count `M`** — `M = array_count × ceil(appends / split_size)`. Every commit re-serializes the snapshot's manifest list (`O(M)`) and resolves each manifest by scanning that list (`O(M²)` per commit in icechunk today), and the snapshot object grows with `M`. Smaller split → more manifests → larger `M`. For a many-variable dataset `M` is dominated by `array_count`, so over-splitting the low-ref-count arrays is the biggest, least useful contributor.
