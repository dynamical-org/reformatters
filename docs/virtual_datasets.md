...
One isolated source-file rejection is discarded. More than one rejection at a rate of at least 10% fails the worker as a systemic source or ref-building failure. The failure reports how many rejections were ambiguous: an absent representative probe or GEFS catalog variable can mean either one incomplete source file or, in a burst, a wrong representative/catalog rule.

A discarded file is removed from the write loop's pending set and is not retried during that run. Readers see fill values at its cells permanently unless a later run explicitly covers the same coordinate. The ERROR line and ingest log's skipped-file count expose the event while the run is live; manifest-completeness validation and availability plots expose the resulting gap afterward.
