...
Fresh-store virtual backfills use the standard parallel temp-branch flow, and overwrite backfills commit straight to `main` alongside operational updates (see [parallel_processing.md](parallel_processing.md#icechunk-stores)). Worker 0 pre-sizes the full template (on the branch, or in place on `main` for an overwrite — a no-op unless the template adds arrays or an explicit extension), so every worker's `sync_dims_to` is a no-op and parallel workers write disjoint refs with no resize conflicts. Jobs partition by chunks along the append dim (virtual arrays have no shards). Workers are assigned contiguous append-dim blocks of jobs (`worker_assignment = "contiguous"`), so each flush rewrites only the manifest windows its own block covers rather than most windows of every array (see [parallel_processing.md](parallel_processing.md#append-dim-region-spreading-and-worker-assignment)).
...
Two notes when backfilling a live virtual dataset (an overwrite backfill):

- **Operational fires keep running.** Backfill workers commit to `main` with a rebase that lets update commits interleave, and an update wins any chunk both wrote (see "Concurrent jobs writing to the same dataset" in [parallel_processing.md](parallel_processing.md)).
- **`append_dim_end` defaults to the store's current end** — the last published position, which is what you want. Explicitly extending past it (requires both overwrite flags) shows NaN-filled slots to readers until the refs land.
