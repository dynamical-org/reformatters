...
A consistently truncated data/index pair passes that internal check, so the write loop also compares each file whose index parsed with like-for-like files at other init times in the same run, using object size and parsed index line count, including files that yielded no refs. When both measures are below half of the peer medians it logs the file and a run-total at ERROR. The detector reports an observation rather than a rejection; any usable refs continue through the normal write path. It deliberately makes no claim without a peer and cannot detect a truncation that retains at least half of either measure or a cohort whose peers share the truncation.

This detector is backfill-only in practice. An operational fire runs `filter_already_present` before ref building and normally retains exactly one new `init_time`; each like-for-like cohort therefore has one member and an empty peer list. Operational coverage requires a separate detector with a durable per-cohort baseline. GEFS truncations also do not reach this detector because GEFS's `_check_refs_complete` assertion runs inside `file_refs` before the observation hook. The two known 2016 HRRR `wrfnat` truncations remain denylisted: 10Z lacks the representative `model_level/temperature` chunk and would fail the ingestion probe, while admitting 12Z would permanently mark its roughly 2%-complete contents as ingested.
