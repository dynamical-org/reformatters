...
2. `discover_available(pending) -> [(coord, file_size)]` — given the source files not yet ingested, return those available to fetch right now, each with its data-file size. Obstore-listable backends one-line this via `discover_available_by_obstore_listing` (below); other sources implement it directly. Withholding a file that *is* available defers it — it stays pending and is offered again next tick — which is how a dataset keeps files that have to land together in one commit. `ingested_through`, the newest append-dim label the store covered when the run started, separates a file that fills in an existing position from one that extends the array. A forecast dataset gates nothing — every file's refs publish as soon as they are built, and the newest init time is normally partially filled by design. An analysis dataset whose positions draw on several lead times follows the opposite convention: data from leads beyond the analysis / lead-time-zero step is held until that position's lead-time-zero file is available, so the leading edge is never ragged (`noaa/hrrr/analysis_virtual`). A dataset virtual over a Zarr source holds a position until all of its source chunks exist (`google/weathernext2/forecast_virtual`).
...
Per-tick commit latency is dominated by the manifest read-modify-write of every array the tick's files touch (see [Manifest splitting](#manifest-splitting) for the cost model). Every icechunk repo is opened with `ManifestConfig(max_concurrent_manifest_fetches_during_commit=16)` so the per-array manifest fetches overlap.
...
- **Operational commit latency** — rewriting a split is a full read-modify-write: flush fetches the active manifest, decodes every ref in it, merges the new refs, re-serializes and re-compresses, so the cost tracks the refs already in the split rather than the refs being added, and every array the commit touches pays it. The metric to size against is **refs per commit = arrays touched × refs per active split**. Production reference point: HRRR forecast 48 hour virtual commits ~12.1 M refs per update and measures 2.8 s median / 4.8 s p95 end-to-end update latency over 12,936 samples. Fewer arrays tolerate more refs per split.
...
Size for operational latency first; then avoid manifests so small that the archive becomes many tiny objects, which slows listings and multiplies requests. Backfill CAS contention is a distant third — 10 backfill workers is usually plenty of parallelism. Weigh these against each other rather than solving for a number.
...
To convert a size budget into a `split_size` you need **bytes per ref**. Measured on HRRR-virtual: **~16.4 bytes/ref** (weighted; median ~17) for manifests holding at least 1000 refs. This is a rough number from a single dataset — location compressibility varies with URL structure, so **check it on each new dataset** once some data is written: `repo.list_manifest_files(snapshot_id)` returns every live manifest's exact `size_bytes` and `num_chunk_refs`. Don't estimate from S3 object sizes (partially-filled splits skew low, and you can't tell fullness from the outside). Two gotchas: icechunk trains a zstd dictionary for locations only at ≥ `min_num_chunks` refs per manifest (default 1000); a manifest below that is smaller per ref (~13 bytes) and skips the training cost, so the ~1000-ref floor on a group's split is about not creating many tiny objects, not compression; and refs are only this small when a `VirtualChunkContainer` prefix covers the locations.
