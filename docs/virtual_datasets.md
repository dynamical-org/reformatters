...
- `out_loc() -> {dim: label}` — the output slab the file fills: a label for each dim the file pins (e.g. init_time and lead_time), omitting dims the file spans wholly (a vertical level dim, when the file carries every level). The inherited default works when the coord's fields are all dim names; override otherwise.
...
**Optional overrides (working defaults):** `filter_already_present` (manifest probe), `representative_var` and `representative_probe_loc` (together they pick the single cell whose manifest presence means the whole file is ingested — per-file commit atomicity makes any one cell imply the rest; the defaults take the first instant var among the coord's own `data_vars`, at `out_loc` plus the first label along each of its multi-chunk dims the file spans wholly, e.g. a vertical level — only override for a packing those rules miss), the `tick_interval` (1s) / `download_concurrency` (64) class attrs, and `process_virtual_refs` itself (only for a fundamentally different batching policy).
