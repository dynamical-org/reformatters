...
For floating-point data variables, Zarr `fill_value` and CF `_FillValue` are equal. Materialized datasets use NaN for both and physically replace an authoritative source fill value declared in `internal_attrs.source_fill_value` (with `source_fill_value_atol` when packing requires a tolerance) with NaN before writing. Virtual datasets cannot change referenced source bytes, so a variable with an authoritative source fill value uses that value for both metadata fields and relies on CF decoding to expose NaN. A published `missing_value` attribute is retained for backward compatibility where it remains truthful, but it is not a second source of truth for the encoding. Materialized scaling is applied before values are saved; virtual pointwise scaling uses Zarr's `ScaleOffset` filter, whose decoded value is `encoded / scale + offset`.
