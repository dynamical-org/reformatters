...
- Declare `dims`, `append_dim`, `append_dim_start`, `append_dim_frequency`. `dims` is keyed by group and annotated with the `Dims` alias: `dims: Dims = {ROOT: (...)}` for a single-level dataset, plus one entry per vertical group (e.g. `"pressure_level": (..., "pressure_level")`). Keep the annotation — pydantic rejects an unannotated override of an inherited field, and it is what makes ty check dim and group names against their `Literal` types.
...
Operational validation conventions. `CheckCurrentData(max_delay=...)` sets when each append-dim position is due, counted from that position's own timestamp (typically the validation cron's offset after the cycle it validates), so a tight deadline holds even on an off-schedule run. `CheckRecentNans` defaults to covering every append-dim position the operational update writes, derived from that update's own jobs, so no dataset restates its schedule as a window number. Writes are shard granular, so an update rewrites its whole trailing shard — hundreds of positions. `CheckExpectedShards` proves a written shard exists; only the NaN check sees NaNs left *inside* a shard by a transiently failed source file. With the default `spatial_sampling="random_points"` that full depth is nearly free — points are drawn once and read across the whole window in one pass, so each zarr chunk is fetched once — while `"quarter"` and `"all"` read each position separately and cost in proportion to depth, so those set an explicit `window` with a comment on the cost. Every result reports how many of the written positions it covered.
