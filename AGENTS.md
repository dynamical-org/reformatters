...
3. Operational updates to the zarr are run using a kubernetes cronjob and validated by another kubernetes cronjob which runs after the update is expected to succeed. Updates use the same parallel worker model as backfills. To ensure the archive is valid to readers throughout the update, zarr v3 metadata is written only after all workers finish, and extent-extending icechunk jobs (updates, expansion and new-store backfills) use a temporary branch that is atomically merged to main; virtual updates commit directly to main. Overwrite backfills that preserve the store's extent run concurrently with operational updates: they defer the region updates rewrite to them and publish cooperatively onto main, with the update winning any conflict (see docs/parallel_processing.md).
