...
def test_httpx_get_with_retry_gives_up_when_the_first_wait_spends_the_budget(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[str] = []
    ...
    limiter = download_module.RateLimiter(max_per_minute=60)
    monkeypatch.setattr(limiter, "wait", lambda: next(clock))
    monkeypatch.setattr(download_module, "_httpx_client", Client)
    monkeypatch.setattr(download_module.time, "monotonic", lambda: next(clock))
    with pytest.raises(TimeoutError, match="retry budget"):
        download_module._httpx_get_with_retry(
            "http://x/y", rate_limiter=limiter, retry_timeout=1.0
        )
    assert calls == []
