...
class _FollowUpJob(VirtualTestRegionJob):
    """Drives the base poll loop; every file is available at once, and each committed
    lead-0 file schedules its init's lead-6h file as follow-up work."""

    process_virtual_refs = VirtualRegionJob.process_virtual_refs
    committed_snapshot_counts: ClassVar[list[int]] = []
    repo: ClassVar[icechunk.Repository | None] = None
    ...
    def committed(
        self, batch: Sequence[tuple[VirtualTestSourceFileCoord, Sequence[VirtualRef]]]
    ) -> Sequence[VirtualTestSourceFileCoord]:
        assert self.repo is not None
        self.committed_snapshot_counts.append(_snapshot_count(self.repo))
        return [
            VirtualTestSourceFileCoord(
                init_time=coord.init_time, lead_time=LEAD_TIMES[1]
            )
...
def _follow_up_job(template_ds: xr.DataTree, repo: icechunk.Repository) -> _FollowUpJob:
    _FollowUpJob.committed_snapshot_counts = []
    _FollowUpJob.repo = repo
    return _FollowUpJob(
        tmp_store=Path("unused-tmp.zarr"),
        template_ds=template_ds,
        data_vars=[VirtualTestDataVar(name="temperature_2m")],
...
def test_committed_follow_ups_are_written_after_the_commit(tmp_path: Path) -> None:
    dataset = _make_dataset(tmp_path)
    template_ds = _create_template_ds(4)
    template_utils.write_metadata(template_ds, dataset.store_factory)
    repo = _primary_repo(dataset.store_factory)
    job = _follow_up_job(template_ds, repo)
    before = _snapshot_count(repo)

    lead_0_files = [c for c in job.source_file_coords() if c.lead_time == LEAD_TIMES[0]]
    job.process_virtual(repo, [], "main", lead_0_files)

    # One commit for the four lead-0 files, then one for their four follow-ups; the
    # hook saw the first commit already landed when it scheduled them.
    assert _snapshot_count(repo) - before == 2
    assert _FollowUpJob.committed_snapshot_counts == [before + 1, before + 2]
    _assert_all_values(dataset, n_inits=4)


def test_committed_is_not_called_when_the_commit_fails(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    dataset = _make_dataset(tmp_path)
    template_ds = _create_template_ds(4)
    ...
    monkeypatch.setattr(storage, "commit_if_icechunk", failing_commit)
    lead_0_files = [c for c in job.source_file_coords() if c.lead_time == LEAD_TIMES[0]]
    with pytest.raises(RuntimeError, match="commit failed"):
        job.process_virtual(repo, [], "main", lead_0_files)
    assert _FollowUpJob.committed_snapshot_counts == []


def test_unfinished_work_defaults_to_the_manifest_filter(tmp_path: Path) -> None:
    dataset = _make_dataset(tmp_path)
    template_ds = _create_template_ds(4)
    template_utils.write_metadata(template_ds, dataset.store_factory)
    repo = _primary_repo(dataset.store_factory)
    ...
    job = _make_region_job(template_ds, region=slice(0, 4))
    readonly = repo.readonly_session("main").store
    assert job.unfinished_work(readonly) == job.filter_already_present(
        job.source_file_coords(), readonly
    )
    assert len(job.unfinished_work(readonly)) == 3 * N_LEADS


def test_process_worker_jobs_writes_the_unfinished_work_override(
    tmp_path: Path,
) -> None:
    dataset = _make_dataset(tmp_path)
    template_ds = _create_template_ds(4)
    ...
    class ExtraWorkJob(VirtualTestRegionJob):
        """A one-init job that also owes the files of the init after its region."""

        def unfinished_work(
            self, store: icechunk.IcechunkStore
        ) -> list[VirtualTestSourceFileCoord]:
            extra = [
                VirtualTestSourceFileCoord(
                    init_time=APPEND_DIM_START + APPEND_DIM_FREQ, lead_time=lead
                )
                for lead in LEAD_TIMES
            ]
            return [*super().unfinished_work(store), *extra]
    ...
    ExtraWorkJob.process_worker_jobs(
        [job], dataset.store_factory, "main", 0, overwrite_chunks=False
    )
    _assert_store_values(repo.readonly_session("main").store, n_inits=2)
