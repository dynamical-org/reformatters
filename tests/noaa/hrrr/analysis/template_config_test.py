...
def test_template_config_attrs() -> None:
    ...
    assert config.append_dim_start == pd.Timestamp("2018-09-16T00:00")
...
def test_dimension_coordinates() -> None:
    ...
    assert (
        dim_coords["time"]
        == pd.date_range("2018-09-16T00:00", "2018-09-16T00:00", freq="1h")
    ).all()
...
def test_derive_coordinates_integration() -> None:
    ...
    config = NoaaHrrrAnalysisTemplateConfig()
    template_ds = config.get_template(pd.Timestamp("2018-09-16T12:00"))

    assert (
        template_ds.coords["time"]
        == pd.date_range("2018-09-16T00:00", "2018-09-16T11:00", freq="1h")
    ).all()
