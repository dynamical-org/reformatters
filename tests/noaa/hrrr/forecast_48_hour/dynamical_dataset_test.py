...
def test_validators(dataset: NoaaHrrrForecast48HourDataset) -> None:
    ...
    assert validators[2].exclude_vars == (
        "percent_frozen_precipitation_surface",
        "geopotential_height_cloud_ceiling",
    )
    assert isinstance(validators[3], validation.CheckRecentNans)
    assert validators[3].include_vars == validators[2].exclude_vars
    assert validators[3].max_nan_fraction == 0.9999
    assert validators[3].spatial_sampling == "quarter"
