...
def test_derive_coordinates(template_config: GefsForecast35DayTemplateConfig) -> None:
    ...
    assert ingested_length[0] == ("init_time", "lead_time")
    assert isinstance(ingested_length[1], np.ndarray)
    assert ingested_length[1].shape == (len(init_times), len(lead_times))
...
def test_coordinates_configuration(
    ...
    assert ensemble_coord.encoding.shards == 31
    ...
    assert lead_time_coord.encoding.chunks == lead_time_coord.encoding.shards
