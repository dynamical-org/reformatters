...
def template_ds() -> xr.DataTree:
    return TEMPLATE_CONFIG.get_template(pd.Timestamp("2024-06-03T00:00"))
...
def make_job(
    ...
    return NoaaGefsForecast35Day05DegreeVirtualRegionJob(
...
def test_source_file_coord_url_names_the_member_and_product(
    ...
    """The extension's lead time is three digits like every other, and both 0.5 degree
    products live beside each other under the cycle."""
    coord = NoaaGefsForecastVirtualSourceFileCoord(
        init_time=pd.Timestamp("2024-06-01T00:00"),
        lead_time=pd.Timedelta("840h"),
        ensemble_member=ensemble_member,
        source_file_type=source_file_type,
        data_vars=[get_var("temperature_2m")],
    )
    assert coord.get_url() == (
        f"s3://noaa-gefs-pds/gefs.20240601/00/atmos/pgrb2{source_file_type}p5/"
        f"{member_str}.t00z.pgrb2{source_file_type}.0p50.f840"
    )
...
def test_every_init_lead_member_triple_gets_one_file_per_product(
    ...
    init_times = [pd.Timestamp("2024-06-01T00:00"), pd.Timestamp("2024-06-02T00:00")]
...
def test_representative_var_is_a_root_instant_variable(
    ...
    coords = coords_for(
        template_ds,
        data_vars,
        init_times=[pd.Timestamp("2021-01-01T00:00")],
        lead_times=[_HOUR_0, pd.Timedelta("3h"), pd.Timedelta("840h")],
...
def test_operational_update_jobs_reach_back_past_the_extension(
    ...
    jobs, template_ds = (
        NoaaGefsForecast35Day05DegreeVirtualRegionJob.operational_update_jobs(
            primary_store=Mock(),
            tmp_store=Path("unused-tmp.zarr"),
    ...
    (job,) = jobs
    assert isinstance(job, NoaaGefsForecast35Day05DegreeVirtualRegionJob)
    assert job.processing_mode == "update"
    init_times = template_ds.to_dataset().get_index("init_time")
    # A 72h window of daily inits.
    assert job.region == slice(len(init_times) - 3, len(init_times))
    swept = init_times[job.region]
    assert swept.max() == pd.Timestamp("2024-06-03T00:00")
    assert now - swept.min() > pd.Timedelta("28h")
...
def test_file_refs_refuses_an_index_missing_a_requested_variable(
    ...
    """At lead 846 the accumulation window is 840-846, so an 834-846 line is a different
    message. Committing the file anyway would leave a NaN column nothing ever retries."""

    def fake_download(url: str, dataset_id: str, *, region: str) -> Path:
        path = tmp_path / "index.idx"
        path.write_text(
            "1:0:d=2024060100:APCP:surface:834-846 hour acc fcst:ENS=low-res ctl\n"
        )
        return path
...
assert len(_IDX_FIXTURES) == 22, _IDX_FIXTURES
...
def test_every_requested_variable_maps_to_a_real_message(
    ...
    element (CLWMR before 2025-12-19, CLMR after), lead 0, the 3 and 6 hour windows, the
    240 hour step up in lead spacing, the 384 hour lead where every other cycle stops,
    the 390 hour lead where only 00z continues, and the 840 hour end, at the archive
    start and at a recent cycle, for perturbed members as well as the control.
    """
