...
def test_cron_job_names_fit_the_kubernetes_limit(
    ...
    """The dataset id plus "-validate" is one character over, so the names abbreviate
    ...
    for cron_job in dataset.operational_kubernetes_resources("test-image-tag"):
        assert cron_job.name.startswith("noaa-gefs-forecast-16-day-0p5-virtual-")
...
def test_validators(
    ...
    # The newest init is 7h55m old when validation fires, so one cycle that rolled its
    ...
    # 105 lead times of 31 members, in both the a and b products.
    expected_files_per_init = 105 * 31 * 2
    allowed_missing = max(
        missing
    ...
    # The last five lead times of every member, which the source lays down in its final
    # stretch; a sixth lead time's worth would fail.
    assert 5 * files_per_lead_time <= allowed_missing < 6 * files_per_lead_time
